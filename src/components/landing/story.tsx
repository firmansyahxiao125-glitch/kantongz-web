...
/**
 * Bagian bercerita — seluruhnya SERVER COMPONENT.
 *
 ...
 * itu di layar kedua.
 *
 * Berkas ini dulu `'use client'` karena satu hal: garis penghubung di bagian
 * "Mesin AI" yang tumbuh mengikuti gulir, digerakkan `useScroll`. Sekarang
 * garis itu digerakkan lini masa gulir CSS (`.rail-host` / `.rail-progress`)
 * yang berjalan di utas kompositor — lebih mulus daripada versi JavaScript-nya,
 * dan tanpa memaksa empat bagian teks di sekitarnya ikut ke peramban.
 */
...
export function MesinAi() {
  return (
    <section id="mesin-ai" className="relative overflow-hidden px-4 py-24 sm:py-32">
      {/* `rail-host` mendeklarasikan lini masa; garis di dalamnya membacanya. */}
      <div className="rail-host mx-auto max-w-5xl">
        <Reveal>
        ...
        <div className="relative mt-14 pl-8 sm:pl-12">
          {/* Rel garis, dan bagian yang sudah dilewati.
              ...
              Garis kedua BUKAN hiasan: ia menyatakan bahwa keempat lapis adalah
              satu urutan yang dilewati dari atas ke bawah, bukan empat pilihan
              yang berdiri sendiri. Peramban tanpa lini masa gulir tidak
              mendapatkannya, dan rel abu-abunya tetap menyampaikan urutan yang
              sama. */}
          <div
            aria-hidden
          ...
          <div
            aria-hidden
            className="rail-progress absolute left-[7px] top-2 h-[calc(100%-1rem)] w-px bg-[var(--color-holo)] sm:left-[15px]"
          />

          <div className="space-y-10">
            {LAPISAN_AI.map(({ icon: Icon, tier, name, detail, why, cost }, index) => (
              <Reveal key={name} index={index} className="relative">
                <span
                  aria-hidden
            ...
            <li key={title} className="h-full">
              <Reveal index={index} className="h-full">
                <div className="panel hover-lift flex h-full flex-col p-6">
                  ...
                  <span
                    className="numeric text-4xl font-medium leading-none text-faint/40"
                    aria-hidden
                  >
  ...
  return (
    <section id="suara" className="mx-auto max-w-6xl px-4 py-24 sm:py-32">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-3">
        {KELUARAN_ASISTEN.map(({ q, a, src }, index) => (
          <Reveal key={q} index={index} className="h-full">
            <figure className="panel hover-lift flex h-full flex-col p-6">
